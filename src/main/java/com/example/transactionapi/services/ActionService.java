...
//        List<Schedule> listSchedule = scheduleRepository.findAllByPaymantdateBeforeAndStatusNotOrderById(LocalDateTime.now(), Status.DONE);
//        for(Schedule schedule: listSchedule){
//            Duration duration = Duration.between(LocalDateTime.now(), schedule.getPaymantdate());
//            float day = ((float) Math.abs(duration.toDays())+1)/365;
//            float penalty = (day * schedule.getMonthly() * 12)/100;
//            schedule.setPenalty(penalty);
//            schedule.setMonthly(schedule.getMonthly()+penalty);
//            scheduleRepository.save(schedule);
//        }
//    }
}
